/*
  # Fix equipment instances

  1. New Functions
    - `generate_equipment_instances` - Creates instances for equipment with individual QR codes
    - `sync_equipment_instances` - Ensures instances match equipment quantity
  
  2. Triggers
    - Add trigger to sync instances when equipment is updated
    
  3. Data Fixes
    - Adds missing instances for existing equipment
*/

-- Function to generate instances for equipment
CREATE OR REPLACE FUNCTION generate_equipment_instances(equipment_id UUID)
RETURNS VOID AS $$
DECLARE
  v_equipment RECORD;
